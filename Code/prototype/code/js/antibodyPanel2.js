...
setGradient(colGradient, "#4b0082", getColorScalePos(350), 0.9); //violet
setGradient(colGradient, "#00c9c9", getColorScalePos(445), 0.9); //blue
setGradient(colGradient, "#16e500", getColorScalePos(475), 0.9); //cyan
setGradient(colGradient, "#d7ea00", getColorScalePos(560), 0.9); //green
setGradient(colGradient, "#fcc100", getColorScalePos(600), 0.9); //yellow
setGradient(colGradient, "#ea0000", getColorScalePos(675), 0.9); //red
setGradient(colGradient, "#7f0800", getColorScalePos(800), 0.9); //dark red
        ...
        for (var j in excitedByLaser){
            ...
            d3.select("#fl"+i+j).append("rect")                    
                    .attr("class", function(d){ 
                        //if (!abInfo[d.name]['ex']){ return "fluor NA"; } 
                        return "spectrum " + exFl + emFl; 
                    })
                    ...
                    .append("title")  // quick tooltip, will upgade
    			    .html( excitedByLaser[j] )
                    ;                        
        }
    }
}
...
//         else { return true;} 
//     });
//     d3.selectAll(".spectrum").attr("opacity", 0.15);
//     d3.selectAll(".fluor").attr("opacity", 0.15);
//     d3.selectAll(".selected").attr("opacity", 0.9);
// }

function mouseover(){
    //console.log(d3.select(this)[0]);
    //console.log(d3.select(this)[0][0].classList[1]);
    var selectedClass = d3.select(this)[0][0].classList[1];
    d3.selectAll(".spectrum").attr("opacity", 0.15);
    d3.selectAll(".fluor").attr("opacity", 0.15);
    d3.selectAll("."+selectedClass).attr("opacity", 0.9);
}
...
function mouseout(){
    ...
    //console.log(d3.select(this)[0][0].classList[1]);
    var selectedClass = d3.select(this)[0][0].classList[1];
    d3.selectAll(".spectrum").attr("opacity", 0.8);
    d3.selectAll(".fluor").attr("opacity", 0.8);
    //d3.selectAll("."+selectedClass).attr("opacity", 0.9);
}
...
function loadAntibodies(){
        ...
        for (var a in abSet){
            if (!abForSp[abSet[a]]){
                ...
                for (var i = 0; i < clones.length; i++){
                    ...
                    colors = colors.concat(fluors); //all colors, may have replicates
                    ...
                    .attr("class", function(d){ 
                        if (!abInfo[d.name]['ex']){ return "fluor NA"; } 
                        return "fluor " + abInfo[d.name]['ex'] + abInfo[d.name]['em']; 
                    })
    ...
    for (var c in colors){
        if (abInfo[colors[c]['name']]['ex']){
            var exEmName = abInfo[colors[c]['name']]['ex'] + abInfo[colors[c]['name']]['em'];
            console.log(exEmName);
            d3.selectAll("." + exEmName)
            .attr("opacity", 0.7)   
